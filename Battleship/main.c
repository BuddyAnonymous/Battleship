                        ...
                        while(counter1 != 2)
                        {
                            ...
                            if(turn1[0] == turn2[0])
                            {
                                for(int i = y1; i <= y2 ; i++)
                                {
                                    if(table[i][x1] != 'O')
                                        counter2++;
                                    if((table[i+1][x1] != 'O' && i<9) || (table[i-1][x1] != 'O' && i > 0) || (table[i][x1+1] != 'O' && x1<9) || (table[i][x1-1] != 'O' && x1>0))
                                    {
                            ...
                            else if(turn1[1] == turn2[1])
                                {
                                    for(int i = x1; i <= x2 ; i++)
                                    {
                                        if(table[y1][i] != 'O')
                                            counter2++;
                                        if((table[y1+1][i] != 'O' && y1<9) || (table[y1-1][i] != 'O' && y1>0) || (table[y1][i+1] != 'O' && i<9) || (table[y1][i-1] != 'O' && i>0))
                                        {
                                            printf("Nije moguce postaviti brod jedan pored drugog!\n");
                                            counter2++;
                                        }
                ...
                else
                {
                    while (counter == 0)
                    {
                        while(counter1 != 1)
                        {
                            ...
                            if(table[y1][x1] == 'O')
                                counter1++;
                            if((table[y1+1][x1] != 'O' && y1<9) || (table[y1-1][x1] != 'O' && y1>0) || (table[y1][x1+1] != 'O' && x1<9) || (table[y1][x1-1] != 'O' && x1>0))
                            {
                                printf("Nije moguce postaviti brod jedan pored drugog!\n");
                                counter1++;
                            }
...
void pravila()
{
    ...
    printf("3. Postavljanje brodova traje dok ne unesete sve brodove i unesete opciju 0 za zavrsetak postavke\n");
    printf("4. Borite se brotiv bota, svaki napad unosite koordinatu za napad, ukoliko ste pogodili brod igrate opet\n");
    printf("5. Pobednik je onaj koji prvi potopi protivniku sve brodove\n\n\n");
}
